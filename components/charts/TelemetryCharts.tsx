...
// Chart colors for multi-lap overlay
const LAP_COLORS = [
  '#E10600', // IRA Red
  '#FFD700', // Gold
  '#3B82F6', // Blue
...
export function TelemetryCharts({
  ...
  // Prepare data for charts - group by lap and normalize by distance
  const chartData = useMemo(() => {
    const selectedPoints = telemetryPoints.filter(p => 
      selectedLapIds.includes(p.lap_id)
    )

    if (selectedLapIds.length === 1 || !showOverlay) {
      // Single lap view - simple array
    ...
    // Multi-lap overlay - need to merge by distance
    ...
    selectedLapIds.forEach((lapId, lapIndex) => {
      ...
      lapPoints.forEach(p => {
        const distance = Math.round((p.distance_meters || 0) / 5) * 5 // Round to 5m intervals
  ...
  const chartConfigs: Record<ChartType, {
    title: string
    dataKeys: string[]
    yAxisLabel: string
    ...
    speed: {
      title: 'Speed',
      dataKeys: showOverlay && selectedLapIds.length > 1 
        ? selectedLapIds.map((_, i) => `speed_${i}`)
        : ['speed'],
      yAxisLabel: 'km/h',
      domain: [0, 'auto'],
    },
    ...
    gforce: {
      title: 'G-Force',
      dataKeys: showOverlay && selectedLapIds.length > 1 
        ? selectedLapIds.flatMap((_, i) => [`latG_${i}`, `lonG_${i}`])
        : ['latG', 'lonG'],
      yAxisLabel: 'g',
      domain: [-3, 3],
    },
  ...
  const handleMouseMove = useCallback((e: any) => {
    if (e?.activePayload?.[0]?.payload && onPointHover) {
      // Find the actual telemetry point closest to this distance
      const distance = e.activePayload[0].payload.distance
      const point = telemetryPoints.find(p => 
        Math.abs((p.distance_meters || 0) - distance) < 10
      )
    ...
    <div className="h-full flex flex-col">
        ...
        {selectedLapIds.length > 1 && (
          <label className="flex items-center gap-2 cursor-pointer">
            <input
              ...
              className="w-4 h-4 rounded border-ira-carbon-600 bg-ira-carbon-700 text-ira-red focus:ring-ira-red"
            />
      ...
      {selectedLapIds.length > 1 && showOverlay && (
        <div className="flex-shrink-0 flex flex-wrap gap-3 mb-4">
          {selectedLapsInfo.map((lap, index) => (
            <div 
              key={lap.id}
              className="flex items-center gap-2 px-3 py-1.5 rounded-full text-sm"
              style={{ backgroundColor: `${lap.color}15` }}
            >
              <div 
                className="w-3 h-3 rounded-full"
                style={{ backgroundColor: lap.color }}
              />
              <span style={{ color: lap.color }}>
                Lap {lap.number}
                {lap.isBest && ' 🏆'}
              </span>
        ...
        <ResponsiveContainer width="100%" height="100%">
          ...
          >
            ...
            <YAxis 
              ...
              tickFormatter={(v) => activeChart === 'temps' ? `${v}°` : v}
              tick={{ fill: '#6E6E82', fontSize: 11 }}
              label={{ 
            ...
            <Tooltip
              ...
              formatter={(value: number, name: string) => {
                const unit = activeChart === 'speed' ? ' km/h' 
                  : activeChart === 'rpm' ? ' rpm'
                  : activeChart === 'temps' ? '°C'
                  : ' g'
            ...
            {showOverlay && selectedLapIds.length > 1 ? (
              ...
              config.dataKeys.map((key, index) => {
                ...
                return (
                  <Line
                    ...
                    name={`Lap ${selectedLapsInfo[lapIndex]?.number || lapIndex + 1}`}
                  />
                )
              })
            ) : (
              // Single lap
              config.dataKeys.map((key, index) => {
                const colors = activeChart === 'gforce' 
                  ? ['#3B82F6', '#22C55E'] // Blue for lateral, Green for longitudinal
                  : activeChart === 'temps'
                  ? ['#EF4444', '#3B82F6'] // Red for exhaust, Blue for water
                  : [LAP_COLORS[0]]
                ...
                return (
                  <Line
                    ...
                    type="monotone"
                    dataKey={key}
                    stroke={colors[index] || LAP_COLORS[0]}
                    strokeWidth={2}
                    dot={false}
                    activeDot={{ r: 4 }}
                    name={key === 'latG' ? 'Lateral G' 
                      : key === 'lonG' ? 'Longitudinal G'
                      : key === 'exhaust' ? 'Exhaust'
                      : key === 'water' ? 'Water'
                      : config.title}
                  />
      ...
      {/* Chart Legend for G-Force / Temps */}
          ...
          {activeChart === 'gforce' ? (
            <>
              <div className="flex items-center gap-2">
                ...
                <span className="text-sm text-white/60">Lateral G (cornering)</span>
              </div>
              <div className="flex items-center gap-2">
                <div className="w-3 h-3 rounded-full bg-green-500" />
                <span className="text-sm text-white/60">Longitudinal G (accel/brake)</span>
              </div>
            </>
          ) : (
            <>
              <div className="flex items-center gap-2">
                ...
                <span className="text-sm text-white/60">Exhaust Temp</span>
              </div>
              <div className="flex items-center gap-2">
                <div className="w-3 h-3 rounded-full bg-blue-500" />
