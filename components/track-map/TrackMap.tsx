...
export function TrackMap({
  ...
  useEffect(() => {
    ...
    mapInstance.on('load', () => {
      ...
      mapInstance.fitBounds(
        [[bounds.minLng - 0.001, bounds.minLat - 0.001], [bounds.maxLng + 0.001, bounds.maxLat + 0.001]],
  ...
  useEffect(() => {
    ...
    if (mapInstance.getLayer('track-line')) mapInstance.removeLayer('track-line')
    if (mapInstance.getSource('track')) mapInstance.removeSource('track')
  ...
  useEffect(() => {
    ...
    mapInstance.addLayer({
      ...
      type: 'circle',
      source: 'beacons',
      paint: { 'circle-radius': 8, 'circle-color': '#FFD700', 'circle-stroke-width': 2, 'circle-stroke-color': '#15151E' },
    })
    mapInstance.addLayer({
      id: 'beacon-labels',
      type: 'symbol',
    ...
    return (
      <div className={`flex items-center justify-center bg-ira-carbon-800 rounded-xl ${className}`}>
        <div className="text-center text-white/50 p-8">
          ...
          <p className="text-sm">Points: {telemetryPoints.length} | With GPS: {telemetryPoints.filter(p => p.latitude && p.longitude).length}</p>
        </div>
      </div>
    )
  }
    ...
    <div className={`relative rounded-xl overflow-hidden ${className}`}>
      ...
      {interactive && (
        <div className="absolute top-4 left-4 flex gap-1 p-1 rounded-lg bg-ira-carbon-800/90 backdrop-blur-sm">
          <button onClick={() => setMapStyle('dark')} className={`px-3 py-1.5 rounded text-sm font-medium transition-colors ${mapStyle === 'dark' ? 'bg-ira-red text-white' : 'text-white/60 hover:text-white'}`}>Dark</button>
          <button onClick={() => setMapStyle('satellite')} className={`px-3 py-1.5 rounded text-sm font-medium transition-colors ${mapStyle === 'satellite' ? 'bg-ira-red text-white' : 'text-white/60 hover:text-white'}`}>Light</button>
        </div>
      )}
      {showHeatmap && (
        <div className="absolute bottom-4 left-4 p-3 rounded-lg bg-ira-carbon-800/90 backdrop-blur-sm">
          ...
          <div className="flex items-center gap-2">
            <div className="flex items-center gap-1"><div className="w-3 h-3 rounded-full" style={{ backgroundColor: SPEED_COLORS.slow }} /><span className="text-xs text-white/80">Slow</span></div>
            <div className="flex items-center gap-1"><div className="w-3 h-3 rounded-full" style={{ backgroundColor: SPEED_COLORS.medium }} /><span className="text-xs text-white/80">Mid</span></div>
            <div className="flex items-center gap-1"><div className="w-3 h-3 rounded-full" style={{ backgroundColor: SPEED_COLORS.fast }} /><span className="text-xs text-white/80">Fast</span></div>
          </div>
